...
# from backend.old.transcribe_video import Config
# from authlib.integrations.flask_client import OAuth
# import uuid, wave, traceback
...
# from backend.old.transcribe_video import convert_srt_to_ass, burn_ass_subtitles
#
# # --- Load .env ---
# import os
...
#     """
#     מוריד קובץ מ־R2 (או כל URL) ושומר אותו בתיקיית uploads.
#     מזהה את סוג הקובץ לפי ה־Content-Type אם אפשר.
#
#     :param task_id: מזהה משימה (לשם שמירת הקובץ)
#     :param url: כתובת URL לקובץ להורדה
#     :param save_dir: תיקייה לשמירת הקובץ
#     :return: הנתיב המלא לקובץ המקומי
#     """
#     os.makedirs(save_dir, exist_ok=True)
#
#     # ניסיון לנחש את הסיומת
#     extension = "bin"
#     if ".mp4" in url:
#         extension = "mp4"
#     elif ".srt" in url:
...
#
#             # אם לא הצלחנו לנחש מה-URL, ננסה מה-Content-Type
#             content_type = response.headers.get("Content-Type", "")
#             if extension == "bin":
#                 if "video" in content_type:
#                     extension = "mp4"
#                 elif "text" in content_type or "srt" in url:
#                     extension = "srt"
#                 else:
#                     extension = "bin"
#
...
#                         f.write(chunk)
#
#         print(f"✅ נשמר בהצלחה: {local_path} ({os.path.getsize(local_path)} bytes)")
#         return local_path
#
#     except requests.exceptions.RequestException as req_err:
#         print(f"❌ שגיאה בבקשה להורדה: {req_err}")
#         raise
#
...
# print("🔑 SECRET_KEY loaded (prefix):", secret[:6])
#
...
#
# # === קונפיגורציה של Flask ===
# app.config.update(
#     UPLOAD_FOLDER='uploads',
#     RESULT_FOLDER='results',
#     SESSION_COOKIE_SAMESITE="None" if IS_PROD else "Lax",
#     SESSION_COOKIE_SECURE=IS_PROD,
#     MAX_CONTENT_LENGTH=500 * 1024 * 1024  # עד 500MB
# )
#
# # === Rate Limiting ===
# limiter = Limiter(
...
#     region_name="",  # R2 לא דורש region
# )
#
#
# def generate_presigned_url(object_key: str, expiration=600):
#     """
#     מייצר כתובת זמנית לקובץ מ־R2.
#
#     :param object_key: הנתיב היחסי בקובץ ב־R2
#     :param expiration: תוקף השעה (ברירת מחדל: 10 דקות)
#     :return: URL חתום או None אם כשל
#     """
...
#     try:
#         # 🔍 חיפוש במסד נתונים לפי task_id או id
...
#             print(f"❌ תמלול לא נמצא עבור task_id={task_id}")
...
#                 print(f"⚠️ גישה לא מורשית: {current_email} מנסה לגשת לקובץ של {owner_email}")
#                 return jsonify({"error": "אין הרשאה לצפות בקובץ זה"}), 403
#
#         # 📂 קבלת הקובץ מ־R2
#         bucket_name = os.getenv("R2_BUCKET_NAME")
#         object_key = f"{task_id}/{filename}"
#         print(f"⬇️ ניסיון הורדת קובץ: bucket={bucket_name}, key={object_key}")
#
#         obj = s3_client.get_object(Bucket=bucket_name, Key=object_key)
#         data = obj['Body'].read()
#
#         content_type = mimetypes.guess_type(filename)[0] or "application/octet-stream"
#         disposition = "inline" if content_type.startswith("video/") else "attachment"
#
#         headers = {
...
#
#         print(f"✅ קובץ נשלח בהצלחה: {filename}, size={len(data)} bytes")
#         return Response(data, headers=headers)
#
#     except s3_client.exceptions.NoSuchKey:
#         print(f"❌ הקובץ לא נמצא ב-R2: {object_key}")
#         return jsonify({"error": "File not found"}), 404
#     except Exception as e:
#         print(f"❌ שגיאה כללית ב-proxy_result_file: {e}")
#         import traceback
...
# from backend.old.transcribe_video import generate_outputs_from_srt
...
#
#         transcription = transcriptions_collection.find_one({"id": transcription_id})
#         if not transcription:
#             return jsonify({"error": "תמלול לא נמצא"}), 404
#
#         base_name = transcription.get("base_name") or transcription_id
#         srt_path = f"results/{base_name}.srt"
#
#         # ✍️ כתיבת SRT חדש
...
#
#         # ☁️ העלאה ל־R2
#         upload_file_to_r2(srt_path, f"{transcription_id}/{base_name}.srt")
#
#         # 🧠 עדכון מסד נתונים
#         transcriptions_collection.update_one(
#             {"id": transcription_id},
#             {"$set": {"content": srt_content, "status": "done"}}
#         )
#
#         # 🛠️ צור פורמטים נוספים והעלה
#         output_paths = generate_outputs_from_srt(srt_path, base_name)
#         for path in output_paths:
#             if not os.path.exists(path):
#                 print(f"⚠️ הקובץ לא נמצא: {path}")
#                 continue
#
#             fname = os.path.basename(path)
#             key = f"{transcription_id}/{fname}"
#             success = upload_file_to_r2(path, key)
#
#             if success:
#                 print(f"✅ הועלה ל־R2: {key}")
#             else:
#                 print(f"❌ כישלון בהעלאה ל־R2: {key}")
#
#         # 🔥 צריבת כתוביות מחדש
#         try:
#             from backend.old.transcribe_video import convert_srt_to_ass, burn_ass_subtitles
#             import tempfile
#             from pathlib import Path
#             from app import generate_presigned_url, download_video_from_r2, get_video_resolution  # ודא שהפונקציות זמינות
#             import os
#
#             srt_key = f"{transcription_id}/{base_name}.srt"
#             video_key = f"{transcription_id}/{base_name}.mp4"
#
#             srt_url = generate_presigned_url(srt_key)
#             video_url = generate_presigned_url(video_key)
#
#             if srt_url and video_url:
#                 with tempfile.TemporaryDirectory() as tmpdir:
#                     tmpdir = Path(tmpdir)
#                     srt_local = download_video_from_r2("edited_subs", srt_url, tmpdir)
#                     video_local = download_video_from_r2("video_original", video_url, tmpdir)
#                     output_path = tmpdir / "burned.mp4"
#
#                     width, height = get_video_resolution(str(video_local))
#                     style = transcription.get("subtitle_style", {})
#                     rtl = transcription.get("language") in ["he", "ar", "fa"]
#
#                     ass_path = tmpdir / "subs.ass"
#                     convert_srt_to_ass(str(srt_local), str(ass_path), resolution=(width, height), style_config=style, rtl=rtl)
#                     burn_ass_subtitles(str(video_local), str(ass_path), str(output_path))
#
#                     # העלאה מחדש ל־R2
#                     upload_file_to_r2(str(output_path), f"{transcription_id}/burned.mp4")
#
#                     # עדכון ה-URL במסד הנתונים
#                     public_url = os.getenv("R2_PUBLIC_BASE")
#                     transcriptions_collection.update_one(
#                         {"id": transcription_id},
#                         {"$set": {"video_with_subs_url": f"{public_url}/{transcription_id}/burned.mp4"}}
#                     )
#
#                     print("🎬 burned.mp4 נוצר והועלה מחדש ✅")
#
#         except Exception as burn_error:
#             print("❌ שגיאה בצריבת כתוביות אחרי עדכון SRT:", str(burn_error))
#
#         return jsonify({"status": "success", "message": "SRT, formats and video updated successfully"})
#
#     except Exception as e:
#         print("❌ update-srt error:", str(e))
#         return jsonify({"error": str(e)}), 500
#
#
#
...
#     is_dev = origin and ("localhost" in origin or "127.0.0.1" in origin)
#
#     # ← שליטה ברורה לפי סביבת הרצה
#     if is_dev:
#         # הרשה רק localhost ב־dev
#         response.headers["Access-Control-Allow-Origin"] = origin or "http://localhost:5174"
#     elif origin == frontend_url:
#         # בפרודקשן - רק הדומיין של הפרונטנד
#         response.headers["Access-Control-Allow-Origin"] = origin
#     else:
#         # אם אין origin או לא מזוהה - אל תאפשר CORS (חסום)
...
#
#         from mongo_client import users_collection
#         from email_utils import send_welcome_email  # ✅ תוסיף את זה למעלה בקובץ אם לא קיים
#
#         # בדיקה אם המשתמש כבר קיים במסד
#         existing_user = users_collection.find_one({"_id": user_info["email"]})
...
#         )
#
#         # ✅ אם המשתמש חדש – שלח מייל ברוך הבא
#         if not existing_user:
#             print("📨 משתמש חדש - שולח מייל ברוך הבא ✅")
#
#             send_welcome_email(
#                 to=user_info["email"],
#                 name=user_info.get("name", "משתמש"),
#                 dashboard_link=os.getenv("FRONTEND_URL", "http://localhost:5174")
#             )
#
...
#     data = request.get_json()
#
#     # אם קיים task_id → השתמש בו גם בתור id
#     task_id = data.get("task_id")
#     data["id"] = data.get("task_id") or data.get("id", str(uuid.uuid4()))
#
